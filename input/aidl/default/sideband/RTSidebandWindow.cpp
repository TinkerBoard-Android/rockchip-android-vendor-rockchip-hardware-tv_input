...
status_t RTSidebandWindow::setBufferGeometry(int32_t width, int32_t height, int32_t format) {
    DEBUG_PRINT(3, "%s %d width=%d height=%d in", __FUNCTION__, __LINE__, width, height);
...
void RTSidebandWindow::messageThreadLoop() {
    ...
    while (mThreadRunning) {
        ...
        DEBUG_PRINT(3, "@%s, finish message id:%d", __FUNCTION__, msg.id);
...
status_t RTSidebandWindow::handleRenderRequest(Message &msg) { 
    ...
    /////////waha mVopRender->SetDrmPlane(0, mSidebandInfo.right - mSidebandInfo.left, mSidebandInfo.bottom - mSidebandInfo.top, buffer, FULL_SCREEN, HDMIIN_TYPE_HDMIRX);
